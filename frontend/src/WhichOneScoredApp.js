      ...
      {comparisonStep !== -2 && (
        <>
          <p>
            ...
            appeared next index the original text. The other one was generated
            by a language model. How confident are you that <b>token A</b> is
            the one that appeared in the original text?
          </p>
          <p>
            {comparisonStep >= 0 && (
              <>
                Progress:{" "}
                {((100 * comparisonStep) / maxComparisonStep).toFixed(0)}%{" "}
              </>
            )}
          ...
          <p
            className={
              comparisonStep >= 0 ? "prompt prompt-activated" : "prompt"
            }
          >
